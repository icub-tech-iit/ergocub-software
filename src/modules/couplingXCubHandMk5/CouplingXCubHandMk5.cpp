...
bool CouplingXCubHandMk5::convertFromActuatedAxesToPhysicalJointsVel(const yarp::sig::Vector& actAxesPos, const yarp::sig::Vector& actAxesVel, yarp::sig::Vector& physJointsVel) {
    ...
    yarp::sig::Matrix actAxesVelToPhysJointsVelJacobian(nrOfPhysicalJoints, nrOfActuatedAxes);
    evaluateCoupledJointsJacobian(actAxesPos, actAxesVelToPhysJointsVelJacobian);

    physJointsVel = actAxesVelToPhysJointsVelJacobian * actAxesVel;
...
bool CouplingXCubHandMk5::evaluateCoupledJointsJacobian(const yarp::sig::Vector& actAxesPos, yarp::sig::Matrix& actAxesVelToPhysJointsVelJacobian) {
    size_t nrOfPhysicalJoints;
    size_t nrOfActuatedAxes;
    auto ok = getNrOfPhysicalJoints(nrOfPhysicalJoints);
    ok = ok && getNrOfActuatedAxes(nrOfActuatedAxes);
    if (!ok || actAxesPos.size() != nrOfActuatedAxes || actAxesVelToPhysJointsVelJacobian.rows() != nrOfPhysicalJoints || actAxesVelToPhysJointsVelJacobian.cols() != nrOfActuatedAxes) {
        yCError(COUPLINGXCUBHANDMK5) << "convertFromPhysicalJointsToActuatedAxesVel: input vector or output vector have wrong size";
        return false;
    }
